...
shape = (320, 160, 804)
voxel_data = np.reshape(np.fromfile(join(base_path, 'Female-v1.raw'), dtype=np.uint8, count=data_size),
                        newshape=shape,
                        order='F')
...
tissue_mapping_file = join('ImportHanako_tissues.txt')
...
# Set the name
vm.name = 'Hanako'
vm.description = 'Hanako (NICT), Resolution: %2.2fmm x %2.2fmm x %2.2fmm' % (voxel_size[0],
                                                                             voxel_size[1],
                                                                             voxel_size[2])
...
vm.add_voxel_data(short_name='original',
                  name='The original model as imported from the Female-v1.raw file of NICT.',
...
(model_trunk, trunk_mask) = importer.calculate_trunk_model(vm, model_type='complete', z_start=380, z_end=660)
